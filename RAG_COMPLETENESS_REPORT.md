...
- "parallel agents" are now parallelized, but citation/revision evaluation is still limited
...
- `run_parallel_agents()` now executes eligibility and rights agents concurrently and merges outputs deterministically in [backend/agents/graph.py](/c:/Users/prane/P-drive/Praneeth/Nish%20ASSIGNMENTS/undertrial-intelligence/backend/agents/graph.py)
- Concurrency behavior is covered by tests (timing + merged output assertions) in [backend/tests/test_rag_phase1.py](/c:/Users/prane/P-drive/Praneeth/Nish%20ASSIGNMENTS/undertrial-intelligence/backend/tests/test_rag_phase1.py)
...
- Add structured citation verification (authority-to-claim checks) in the critic/revision loop
...
## High ROI Features: 
🧑‍⚖️ Human-in-the-loop lawyer verification UI — implement this first, it's the highest ROI
This is the one that transforms the project from "demo" to "real tool." Right now the critic is an LLM reviewing another LLM's output, which is fundamentally limited. A simple lawyer-facing interface where a human can flag issues, approve the brief, and add notes would make the system genuinely useful and would give you real feedback data to improve everything else. It's also not that hard — a few extra fields in the DB and a review page in the frontend.
📱 Hindi / regional language support — worth it, but scope it carefully
Undertrial prisoners and their families often don't speak English. A plain-language summary in Hindi (or Marathi, given your Maharashtra focus) would be a meaningful improvement with real impact. The plain_summary field already exists — you'd just add a translation step after it's generated, which Gemini handles well natively. This is probably a day of work and has the clearest humanitarian justification.
📊 Metrics — partially worth it, but be honest about what you can measure
"X% of cases eligible for bail" is easy to implement and useful for an NGO dashboard — just aggregate the eligibility_verdict field across cases. "Y days saved per case" is essentially made up unless you have a real baseline from actual lawyers. Don't implement the second metric — it'll look hollow and undermine credibility with anyone who pushes back on it.
🔥 NGO dashboard + case priority scoring — worth it if you have an NGO partner
Priority scoring (detention days over threshold + charges severity + court backlog) is technically straightforward and genuinely useful for triage. But a dashboard without a real user is a UI exercise. The honest advice: don't build this until you have even one NGO or legal aid clinic willing to try the tool, then build exactly what they ask for. Otherwise you're guessing at their workflow.