...
import { listReviews, submitReview, getCaseReview } from '../lib/api_review'
...
// ── Inline brief panel shown when a row is expanded ──────────────────────────
...
function CasePanel({ caseId, onClose }) {
  ...
  // Pull the stored brief from sessionStorage (set by AnalysisPage after analysis)
  useEffect(() => {
    try {
      const stored = sessionStorage.getItem(`brief:${caseId}`)
      if (stored) setBrief(JSON.parse(stored))
    } catch {
      // Brief not cached — show what we have from the review record alone
    }
  }, [caseId])

  // Pull any existing review
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    if (!form.verdict) { setError('Please select a verdict.'); return }
    setSaving(true)
    setError('')
    try {
        ...
        <div className={styles.panelBody}>
          ...
          <div className={styles.reviewForm}>
            ...
            <div className={styles.verdictGroup}>
              {Object.entries(VERDICT_META).filter(([k]) => k !== 'pending').map(([val, meta]) => (
                <label
                  key={val}
            ...
            <textarea
              ...
              placeholder="Notes for the advocate (optional) — e.g. 'Hussainara citation is correct, add section 167 timeline'"
              value={form.note}
            ...
            <div className={styles.reviewFooter}>
              ...
              />
              <button
                className={`${styles.submitBtn} ${saved ? styles.submitSaved : ''}`}
...
export default function ReviewPage() {
  ...
  // Also show cases that have been analyzed this session but not yet reviewed
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>
            Review AI-generated briefs, verify citations, and record your verdict before filing.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <div className={styles.emptyState}>
              ...
              <div className={styles.emptyHint}>
                Analyze a case from the Analysis page — it will appear here for review.
              </div>
            </div>
          ) : (
            filtered.map(row => {
                  ...
                  <div className={styles.caseRowLeft}>
                    ...
                    {row.reviewed_at && (
                      <div className={styles.caseRowDate}>
                        {new Date(row.reviewed_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
